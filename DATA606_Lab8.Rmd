...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    to predict the number of runs?  **I used a scatter plot to display the relationship between 'runs' and 'at_bats'  
...
**Answer:** Yes, the relationship looks linear. The correlation coefficient is large enough to predict 'runs' scored by player from his 'at_bats' number.  
...
**Answer:**  
It looks like there is a positive relationship between 'runs' and 'at_bats'  
There is a positive correlation coeffidient of 0.610627 with a positive direction.  
They are some outliers, but not significant.  
...
**Answer:** Smallest Sum of Square that I got was: 123721.9
...
Based on the data, we see that at 5579 at_bat, runs are 713. We would expect 0.6305 fewer runs for 5578 $713-0.6305=712.3695$  
That means the team manager would over estimate the number of runs with the residual of:
$\hat{e}=y_i - \hat{y}$  
...
**Answer:** There is a linear relationship with some high variation.   
...
**Answer:** This Normal QQ plot shows that the relationship between the theoretical percentile and sample percentile is approximately linear. Therefore, the normal probability plot of the residuals suggests that the residuals are indeed nearly normal.   
...
**Answer:** Yes, the constant variability condition appear to be met.   
...
```{r}
plot_ss(y = mlb11$runs, x = mlb11$hits)
```
...
**Answer:** The relationship does looks linear.  
...
**Answer:** They both have a similar type of relationship but with less outliers at hits than in at_bats.  
...
**Answer:** $R^2$ for hits = 0.6419 and $R^2$ for at_bats 0.3729. Higher $R^2$ for hits indicates that its a good predictor for runs compared to at_bats.   
...
```{r}
#R-squared for strikeouts
m4 <- lm(runs ~ strikeouts, data = mlb11)
summary(m4)$r.squared
```

```{r}
#R-squared for home runs
m5 <- lm(runs ~ homeruns, data = mlb11)
summary(m5)$r.squared
```

```{r}
#R-squared for stolen_bases
...
```{r}
#R-squared for new_onbase  
m7 <- lm(runs ~ new_onbase, data = mlb11)
summary(m7)$r.squared
```

```{r}
#R-squared for new_slug 
m8 <- lm(runs ~ new_slug, data = mlb11)
summary(m8)$r.squared
```

```{r}
#R-squared for new_obs
m9 <- lm(runs ~ new_obs, data = mlb11)
summary(m9)$r.squared
```

**Answer:** By far with the highest $R^2$ of 0.9349271, new_obs is the best predictor for runs in this model. Lets take a closer look at the top 3 $R^2$ for runs, which are new_onbase, new_slug and new_obs for a better understanding.  
...
```{r}
qqnorm(m9$residuals)
qqline(m9$residuals)  # adds diagonal line to the normal prob plot
```